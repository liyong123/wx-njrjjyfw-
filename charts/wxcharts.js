...
function drawToolTipYellow(textList, offset, opts, config, context, labelBgColor) {
  var legendWidth = 0;
  ...
  context.beginPath();
  context.setFillStyle(labelBgColor || '#07b985');
...
function drawToolTipBridge(opts, config, context, process) {
  ...
  if (opts.tooltip && opts.tooltip.textList && opts.tooltip.textList.length && process === 1) {
    drawToolTipYellow(opts.tooltip.textList, opts.tooltip.offset, opts, config, context, opts.labelBgColor);
  }
  context.restore();
}
...
function drawRadarDataPoints(series, opts, config, context) {
  ...
  var _loop = function _loop(i) {
    ...
    // 雷达背景
    context.fillStyle = 'rgba(176,196,255,0.3)';
...
function drawRing(opts, config, context) {
  ...
  context.moveTo(width / 2, -height * 8 / 20 + height / 2);
  if (opts.value * 1 == 0 ) {
    context.arc(width / 2, height / 2, height * 8 / 20, -Math.PI / 2, - Math.PI / 2);
  } else {
    context.arc(width / 2, height / 2, height * 8 / 20, -Math.PI / 2, -Math.PI * 2 * opts.value * process / 5 - Math.PI / 2, true);
  }
...
function drawStar(opts, config, context) {
  ...
  var R = height / 2.5;
  var r = height / 5;
  var space = (width - R * 2 * 5) / 4;
  context.beginPath();
  drawSingleStar(context, r, R, R, height / 2, 0);
  drawSingleStar(context, r, R, R + (R * 2 + space) * 1, height / 2, 0);
  drawSingleStar(context, r, R, R + (R * 2 + space) * 2, height / 2, 0);
  drawSingleStar(context, r, R, R + (R * 2 + space) * 3, height / 2, 0);
  drawSingleStar(context, r, R, R + (R * 2 + space) * 4, height / 2, 0);
  context.closePath();
  ...
  var xiaoshu = value - Math.floor(value);
  if (Math.floor(value) === 0) {
    context.fillRect(0, 0, R * 2 * xiaoshu, height);
  } else {
    context.fillRect(0, 0, (R * 2 + space) * Math.floor(value) + R * 2 * xiaoshu, height);
  }
}
