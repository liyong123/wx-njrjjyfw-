...
var expertListApi = app.globalData.focusExpertListApi; //我关注的专家列表接口
var providerListApi = app.globalData.focusProviderListApi; //我关注的供应商列表接口
...
Page(filter.identityFilter({
  ...
  onPullDownRefresh:function(){
    ...
    if(that.data.currentTab == 0){
      if (that.data.expertList.length > 0) {
        ...
        // console.log("screenHeightP:", (wx.getSystemInfoSync().screenHeight - 85))
      ...
      if (that.data.expertList.length > 0) {
        ...
        // console.log("screenHeight:", (wx.getSystemInfoSync().screenHeight - 85))
        if ((listHeight + 44)< (wx.getSystemInfoSync().screenHeight - 95) ) {
          that.setData({
    ...
    util.callAjaxGetSign(url, params, res => {
      ...
      if (res) {
        if (res.data.resultcode == '0000') {
          ...
          // wx.showModal({
          //   title: "温馨提示",
          //   content: "获取数据失败，请联系客服:025-52821888",
          //   showCancel: false,
          //   confirmText: "关闭"
          // });
          that.setData({
  ...
  byUrlLoadData: function() {
    ...
    if (currentTab == 0) {
      this.loadList(providerUrl)
    } else if (currentTab == 1) {
      this.loadList(expertUrl)
    }
  },
  //跳转到专家详情
